...
@Composable
fun Collection5Screen(
    onNavigateBack: () -> Unit,
    viewModel: Collection5VM = hiltViewModel(),
) {
    val customersWithTotalPurchase = viewModel.getCustomersWithTotalPurchase().collectAsState(
        initial = emptyList()
    ).value
    ...
    Scaffold(
        ...
        Column(
            ...
            Text("Customers with Highest Purchase", fontSize = 20.sp, fontWeight = FontWeight.Bold)
            CustomersListContent(customersWithTotalPurchase)
        }
    }
}

@Composable
fun CustomersListContent(customers: List<CustomerWithTotalPurchase>, modifier: Modifier = Modifier) {
